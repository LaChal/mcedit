    ...
    def editContainer(self, point, containerID):
        ...
        def deleteFromWorld():
            ...
            if d.present() == "OK":
                ...
                progressInfo = _("Deleting the item {0} from the entire world ({1} chunks)").format(itemName(chestWidget.id, 0), self.editor.level.chunkCount)
...
class LevelEditor(GLViewport):
    ...
    def initWindowCaption(self):
        ...
        title = os.path.split(s[0])[1] + os.sep + s[1] + _(u" - MCEdit ") + release.release
    ...
    def saveInfoLabelText(self):
        if self.unsavedEdits == 0:
            return ""
        return _("{0} unsaved edits.  CTRL-S to save.  {1}").format(self.unsavedEdits, "" if self.recordUndo else "(UNDO DISABLED)")
...
#
    ...
    def gl_draw(self):
        ...
        if self.debug > 0:
            self.debugString = _("FPS: %0.1f/%0.1f, CPS: %0.1f, VD: %d, W: %d, WF: %d, ") % (1000000. / (float(timeDelta.microseconds) + 0.000001),
                                                                           self.averageFPS,
                                                                           cps,
    ...
    def createWorkInfoPanel(self):
        ...
        for w in sorted(self.workers):
            if isinstance(w, MCRenderer):
                ...
                progress = Label(_("{0} chunks ({1} pending updates)").format(len(w.chunkRenderers), len(w.invalidChunkQueue)))
    ...
    def updateInspectionString(self, blockPosition):
        ...
        try:
            ...
            self.inspectionString += _("Chunk {0} had an error: {1!r}").format((int(numpy.floor(blockPosition[0])) >> 4, int(numpy.floor(blockPosition[2])) >> 4), e)
            pass
