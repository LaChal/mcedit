...
class FillTool(EditorTool):
    ...
    @property
    def statusText(self):
        return _("Press {hotkey} to choose a block. Press {R} to enter replace mode. Click Fill or press ENTER to confirm.").format(hotkey=self.hotkey, R=config.config.get("Keys", "Roll").upper())
    ...
    def worldTooltipText(self):
        if pygame.key.get_mods() & pygame.KMOD_ALT:
            try:
                ...
                return _("Click to use {0} ({1}:{2})").format(self.editor.level.materials.blockWithID(blockID, blockdata).name, blockID, blockdata)
